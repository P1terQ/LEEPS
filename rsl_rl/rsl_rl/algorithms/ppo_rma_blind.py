...
from rsl_rl.modules import ActorCriticRMA_Blind
from rsl_rl.storage import RolloutStorage

class PPORMA_Blind:
    actor_crtic: ActorCriticRMA_Blind
    def __init__(self,
                 actor_critic,
                 estimator,
                 estimator_params,
    ...
    def act(self, obs, critic_obs, hist_encoding=False):
        ...
        if self.train_with_estimated_states:
            obs_est = obs.clone()
            # 通过本体的proprioception 估计priv_explicit(速度)
            priv_state_estimated = self.estimator(obs_est[: , :self.num_prop])
            # 把原本的priv_explicit替换成估计的priv_explicit
            obs_est[:, self.num_prop:self.num_prop+self.priv_states_dim] = priv_state_estimated
    ...
    def update(self):
        ...
        for obs_batch, critic_obs_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, \
            ...
            #! history adaption module update
            privinfo_batch = self.actor_crtic.actor.infer_privinfo_latent(obs_batch)
            with torch.inference_mode():
                privinfo_batch_estimated = self.actor_crtic.actor.infer_hist_latent(obs_batch)
            privinfo_reg_loss = (privinfo_batch - privinfo_batch_estimated.detach()).norm(p=2, dim=1).mean()
            ...
            estimator_loss = (priv_states_predicted - obs_batch[:,self.num_prop:self.num_prop+self.priv_states_dim]).pow(2).mean() 
            ...
            loss = surrogate_loss + \
                    ...
                    priv_reg_coef * privinfo_reg_loss     
                   
            #! actor critic update 
            self.AC_optimizer.zero_grad()
            loss.backward()
            nn.utils.clip_grad_norm_(self.actor_crtic.parameters(), self.max_grad_norm)
    ...
    def update_dagger(self):
        ...
        for obs_batch, critic_obs_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, \
            ...
            with torch.inference_mode():
                privinfo_latent_batch = self.actor_crtic.actor.infer_privinfo_latent(obs_batch)
