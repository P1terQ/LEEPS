...
class PPORMADepth:
    ...
    def __init__(self,
                 ...
                 estimator,
                 estimator_params,
                 terrain_depth_encoder,
                 ceiling_depthe_encoder,
                 depth_encoder_paras,
                 depth_actor,
                 device='cpu',
                 # algorithm cfg
        ...
        # depth encoder
        self.if_depth = terrain_depth_encoder!=None
        if self.if_depth:
            self.terrain_depth_encoder = terrain_depth_encoder
            self.terrain_depth_encoder_optimizer = optim.Adam(self.terrain_depth_encoder.parameters(), lr=depth_encoder_paras["learning_rate"])
            
            self.ceiling_depth_encoder = ceiling_depthe_encoder
            self.ceiling_depth_encoder_optimizer = optim.Adam(self.ceiling_depth_encoder.parameters(), lr=depth_encoder_paras["learning_rate"])
            
            self.depth_encoder_paras = depth_encoder_paras
            
            self.depth_actor = depth_actor
            #! 同时优化depth_encoder和depth_actor的参数
            self.depth_actor_optimizer = optim.Adam([*self.depth_actor.parameters(), *self.terrain_depth_encoder.parameters(), *self.ceiling_depth_encoder.parameters()], lr=depth_encoder_paras["learning_rate"])
    ...
    def update_depth_both(self, 
                          terrain_depth_latent_batch, ceiling_depth_latent_batch, 
                          terrain_scandots_latent_batch, ceiling_scandots_latent_batch, 
                          actions_student_batch, actions_teacher_batch):
        if self.depth_encoder != None:
            # depth_encoder_loss = (scandots_latent_batch.detach() - depth_latent_batch).norm(p=2, dim=1).mean()
            
            terrain_depth_encoder_loss = (terrain_scandots_latent_batch.detach() - terrain_depth_latent_batch).norm(p=2, dim=1).mean()
            ceiling_depth_encoder_loss = (ceiling_scandots_latent_batch.detach() - ceiling_depth_latent_batch).norm(p=2, dim=1).mean()
            depth_actor_loss = (actions_teacher_batch.detach() - actions_student_batch).norm(p=2, dim=1).mean()
            
            depth_loss = terrain_depth_encoder_loss + ceiling_depth_encoder_loss + depth_actor_loss
            
            self.depth_actor_optimizer.zero_grad()
            depth_loss.backward()
            nn.utils.clip_grad_norm_([*self.depth_actor.parameters(), *self.terrain_depth_encoder.parameters(), *self.ceiling_depth_encoder.parameters()], self.max_grad_norm)
            self.depth_actor_optimizer.step()
            
            return terrain_depth_encoder_loss.item(), ceiling_depth_encoder_loss.item(), depth_actor_loss.item()
